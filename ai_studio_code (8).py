...
def calculate_default_transport(loc_from, loc_to):
    ...
    if dist == 9999: return "📍 移動", 30 # 未知地點預設 30分
    
    # 簡單邏輯判斷
    if dist < 0.02:
        return "🚶 步行", int(dist * 1000) + 5
    elif dist < 0.05:
        return "🚕 計程車", int(dist * 600) + 10
    else:
        return "🚆 電車", int(dist * 800) + 15
...
st.markdown("""
    ...
    /* 編輯模式下的交通編輯區 */
    .transport-edit-box {
        background: rgba(0,0,0,0.02); border: 1px dashed #ccc; border-radius: 8px; padding: 8px; margin-left: 10px; width: 100%;
    }
    ...
    for index, item in enumerate(current_items):
        ...
        if index < len(current_items) - 1:
            ...
            if is_edit_mode:
                ...
                with c_trans1:
                    item['trans_mode'] = st.selectbox("交通方式", TRANSPORT_OPTIONS, 
                                                      index=TRANSPORT_OPTIONS.index(item['trans_mode']) if item['trans_mode'] in TRANSPORT_OPTIONS else 0, 
                                                      key=f"tr_m_{item['id']}", label_visibility="collapsed")
                with c_trans2:
                    item['trans_min'] = st.number_input("分", value=item['trans_min'], step=5, key=f"tr_t_{item['id']}", label_visibility="collapsed")
