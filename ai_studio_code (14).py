...
def add_expense_callback(item_id, day_num):
    ...
    if name and price > 0:
        # 找到對應的 item 並更新 (因為在 callback 中無法直接存取 loop 變數，需透過 session_state 搜尋)
...
with tab1:
    ...
    for index, item in enumerate(current_items):
        ...
        expense_html_list = []
        if item['expenses']:
            expense_html_list.append("<div class='expense-box'>")
            for exp in item['expenses']:
                exp_twd = int(exp['price'] * st.session_state.exchange_rate)
                expense_html_list.append(f"<div class='expense-item'><span>{exp['name']}</span><span>¥{exp['price']:,} (NT${exp_twd})</span></div>")
            expense_html_list.append("</div>")
        expense_block = "".join(expense_html_list)
        ...
        <div style="position:relative;">
            ...
            <div class="itinerary-card">
                ...
                <div class="card-sub">
                    ...
                    <span class="card-tag" style="margin-left:auto;">{price_tag}</span>
                </div>
                {note_html}
                {expense_block}
            </div>
