...
st.markdown(f"""
    ...
    div[role="radiogroup"] label p::first-line {{
        font-size: 0.8rem !important; color: #AAA !important;
    }}
    ...
    button[data-baseweb="tab"] {{
        padding: 10px 20px !important; font-size: 1rem !important; white-space: nowrap !important;
        color: {current_theme['sub']}; border-bottom: 3px solid transparent;
    }}
    button[data-baseweb="tab"][aria-selected="true"] {{
        color: {current_theme['primary']}; border-bottom: 3px solid {current_theme['primary']}; font-weight: bold;
    }}
...
with tab1:
    ...
    for index, item in enumerate(current_items):
        ...
        # 關鍵修復：單行 HTML
        card_html = f"""<div style="position:relative;"><div class="time-label">{item['time']}</div><div class="time-dot"></div><div class="itinerary-card"><div class="card-title">{icon} {item['title']}</div><div class="card-sub"><span>📍 {item['loc']}</span>{map_btn}<span class="card-tag">{price_tag}</span></div>{note_div}{exp_div}</div></div>"""
        ...
        if is_edit_mode:
            with st.container(border=True):
                ...
                try: t_val = datetime.strptime(item['time'], "%H:%M").time()
...
with tab2:
    ...
    if m_items:
        ...
        for i, it in enumerate(m_items):
            ...
            html.append(f"""<div class='map-tl-item' style='animation-delay:{i*0.1}s'><div class='map-tl-icon'>{ic}</div><div class='map-tl-content'><div style='color:{current_theme['primary']}; font-weight:bold;'>{it['time']}</div><div style='font-weight:900; font-size:1.1rem; color:{current_theme['text']};'>{it['title']}</div><div style='font-size:0.85rem; color:{current_theme['sub']};'>{it['loc']}</div></div></div>""")
        html.append('</div>')
