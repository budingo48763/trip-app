    ...
    for item in items:
        ...
        with col_card:
            ...
            if item["type"] == "transport":
                ...
                """, unsafe_allow_html=True) # 關鍵：unsafe_allow_html=True

            # 渲染：景點卡片
            else:
                img_html = f'<img src="{item["image"]}" class="card-img">' if item["image"] else ''
                # 這裡修復了你遇到的問題，確保 HTML 被正確渲染
                st.markdown(f"""
...
# === 頁面 B: 行前準備 (須知 + 清單) ===
    ...
    with tab2:
        ...
        for category, items in st.session_state.packing_list.items():
            ...
            cols = st.columns(2) # 兩欄顯示比較省空間
            for i, (item_name, is_checked) in enumerate(items.items()):
                # 使用 session_state 來記憶勾選狀態
                col_idx = i % 2
