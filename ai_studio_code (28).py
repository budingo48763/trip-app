...
st.markdown(f"""
    ...
    /* 時間軸 (關鍵：使用 Flexbox 佈局來對齊) */
    .timeline-row {{
        display: flex; flex-direction: row; align-items: flex-start; margin-bottom: 0px;
    }}
    .time-col {{
        width: 60px; min-width: 60px; text-align: right; font-weight: bold; font-family: 'Times New Roman';
        color: {current_theme['sub']}; padding-top: 15px; padding-right: 15px; font-size: 1rem;
    }}
    .line-col {{
        width: 20px; min-width: 20px; display: flex; flex-direction: column; align-items: center; position: relative;
    }}
    .line-dot {{
        width: 12px; height: 12px; border-radius: 50%; background: {current_theme['text']};
        border: 2px solid {current_theme['bg']}; margin-top: 20px; z-index: 2;
    }}
    .line-bar {{
        width: 2px; flex-grow: 1; background: transparent; border-left: 2px dashed {current_theme['secondary']};
        min-height: 40px; z-index: 1;
    }}
    .content-col {{
        flex-grow: 1; padding-bottom: 20px;
    }}
    
    /* 交通 Badge */
    .travel-badge {{
        background-color: {current_theme['card']}; border: 1px solid #DDD; border-radius: 6px;
        padding: 4px 8px; font-size: 0.75rem; color: {current_theme['sub']}; margin-left: 80px; margin-bottom: 15px; display: inline-block;
    }}
    ...
    for idx, item in enumerate(day_items):
        ...
        if idx < len(day_items) - 1:
            if is_edit:
                ...
                st.markdown(f'<div class="travel-badge">{item["trans_mode"]} {item["trans_min"]}分</div>', unsafe_allow_html=True)
...
with tab4:
    ...
    if edit_i and st.button("➕ 住宿"):
        st.session_state.hotel_info.append({"id": int(time.time()), "name":"新住宿", "range":"", "date":"", "addr":"",