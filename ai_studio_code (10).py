    ...
    for i, hotel in enumerate(st.session_state.hotel_info):
        if info_edit_mode:
            with st.expander(f"編輯: {hotel['name']}", expanded=True):
                ...
                hotel['link'] = st.text_input("Google Maps 連結", hotel['link'], key=f"h_l_{hotel['id']}")
                if st.button("刪除住宿", key=f"del_h_{hotel['id']}"):
        ...
        <div class="info-card" style="border-left: 5px solid #8E2F2F;">
            <div class="info-header">
                <span class="info-tag" style="background:#8E2F2F; color:white;">{hotel['range']}</span>
